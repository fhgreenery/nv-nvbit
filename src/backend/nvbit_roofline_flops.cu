...
void roofline_flops_nvbit_at_cuda_event(CUcontext ctx, int is_exit, nvbit_api_cuda_t cbid,
    ...
    if (cbid == API_CUDA_cuLaunch || cbid == API_CUDA_cuLaunchKernel_ptsz ||
        ...
        if (!is_exit) {
            ...
            printf(
                "kernel %d - %s - #thread-blocks %d,  per kernel"
                " compute insts %lu\n",
                kernel_id++, nvbit_get_func_name(ctx, func, mangled), num_ctas,
                kernel_instrs);
            fflush(stdout);

            yosemite_gpu_data_analysis(nullptr, kernel_instrs);
            yosemite_kernel_end_callback(nvbit_get_func_name(ctx, func), 0 /* device id */);
